...
1. **F1.1 — Secretos y arranque nativo.** Definir una fuente local ignorada por Git para variables de proceso, sin reutilizar `.env` de Compose. Debe aportar datasource, JWT y política explícita de semillas. Reemplazar los `.bat` y tareas VS Code que invocan Docker por comprobaciones de Java, Node y PostgreSQL nativo, inicio controlado de backend/frontend, health check y cierre por PID.
...
4. **F1.4 — Pruebas sin Testcontainers.** Sustituir las tres pruebas de integración actuales basadas en `PostgreSQLContainer` por una estrategia protegida contra `ruta_fija_test`: perfil dedicado, URL bloqueada contra desarrollo, migración controlada y limpieza segura. El objetivo es que `mvn verify` sea nativo y no requiera Docker.
5. **F1.5 — Frontend y tiempo real.** Configurar el proxy de Angular para `/api` y `/ws` con soporte WebSocket, conservar mismo origen y comprobar REST, login, refresh y `/ws/operations` desde `http://localhost:4200`.
6. **F1.6 — Seguridad local y documentación.** Restringir PostgreSQL a loopback si sigue escuchando en todas las interfaces, actualizar README, manual de uso, tareas VS Code y CI para el camino nativo. Revisar que no haya secretos en Git.
7. **F1.7 — Nueva evidencia de recuperación.** Con V1–V5 aplicadas, generar un nuevo dump, restaurarlo en otra base de recuperación y comparar Flyway, tablas, `btree_gist`, constraints V5 y auditoría. Solo entonces se podrá cerrar la observación de integridad de G0.
